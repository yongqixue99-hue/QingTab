...
状态：后台轻量化、隔离封包、特殊 Shell 位置修复和用户日常操作验收均已完成；2026-08-25 已通过正常退出旧版的方式将当前机器切换到 0.2.6 修复候选。
...
- 行为检查：基础阶段单次 `PASS: 206`；特殊 Shell 位置修复后扩展为 `PASS: 218`。
...
- 特殊 Shell 位置修复后 EXE：`117248` 字节，仅比第一阶段增加 `512` 字节，仍约 `0.11 MB`；SHA-256：`65758B48A71E05A031D77ED817B52653258FC59ABB7A5CC09E2372B5B408D42D`。
...
- 从特殊 Shell 位置修复后的源码包独立目录显式构建产品与测试项目均为 `0` 警告、`0` 错误，行为检查为 `PASS: 218`，生命周期检查为 `PASS: 23`。不同绝对目录下重建的 EXE 大小与版本一致，但 SHA-256 不同，因此当前只承诺确定性编译设置，不宣称跨路径逐字节可复现。
...
- 2026-08-25 用户发现回收站无法进入。根因是 Folder 默认打开接管同时覆盖了回收站等虚拟 Shell 文件夹，而客户端与驻留端都没有在入队前分流。现已改为只接管绝对盘符路径和 UNC 路径；回收站、控制面板、库、`shell:` 与 CLSID 解析名直接交给 `explorer.exe` 原生处理。专项检查覆盖正常盘符、盘符根目录解析残留、UNC、回收站两种解析名、控制面板、库、相对路径和原生回退参数。
...
- 唯一 QingTab 驻留为 `QingTab-v0.2.6-portable\QingTab.exe`，版本 `0.2.6.0`；公开报告不记录本机绝对路径。
- `HKCU Run`、Folder 默认打开命令与所有权记录均已指向该 0.2.6 修复候选，`DelegateExecute` 仍为空。
- 切换时旧版通过自身 `--exit` 正常恢复 Windows 打开方式并退出；随后才由新版重新写入接管并以隐藏启动方式驻留。没有打开、导航、关闭或切换任何 Explorer 窗口。

## 发布结论

- 直接路径仍保留导航结果分类、Shell 重连、操作生命周期、300 ms 去抖、有界 FIFO 和安全退出恢复；静态程序集审计未发现被撤回的视觉类型或 UIAutomation 引用。
- 回收站误接管回归已在 0.2.6 候选中修复并锁定为专项检查；已知的 Explorer 默认页过渡已写入说明，不再宣传成“真正零闪烁”。
